...
| **Échéance OBLIGATOIRE sur toute activité + journalisation des changements** (doctrine Médéa : « toute activité doit avoir une échéance, sinon le suivi est infernal ; un changement d'échéance doit être documenté immédiatement »). Impossible d'enregistrer sans date sur : **actions de suivi**, **actions ESG**, **création de valeur**, **points de due diligence** (migration 0045 : colonne `due_date`, date exigée dès la constitution de la liste). Éléments sans date repérés en rouge « échéance à définir ». **Comités — décision à valider** : remontent **dès la proposition** avec un délai par défaut de **3 jours ouvrables** comme date, « en retard » au-delà (`todo.ts`). Tout changement d'une échéance existante exige un **motif** et écrit une **note datée** au journal du Suivi (helper `src/lib/suivi-log.ts`). **La to-do du tableau de bord = liste des « actions à prendre »** (à venir ET en retard, affichées « dans X jours » / « en retard ») pour ÉVITER les retards — plus aucun « sans échéance ». Défaut « Mes actions » (liste personnelle) ; **« Tout le fonds » (liste complète) visible par tous les rôles INTERNES, pas les externes (Auditeur, Observateur / LP)** — `canSeeAll = !isExternalRole(roleName)`. Deux groupes : **urgent** (retard ou < 7 j) affiché, **plus tard** sous un bouton « + ». | à tester | 0045 |
...
| Alerte sur compte dormant ou accès jamais utilisé — **livrée** dans le journal (onglet Activité de l'équipe) : bandeau « X personne(s) ne se sont pas connectées depuis N jours… un compte inutilisé est un accès ouvert pour rien », conditionnel (ne s'affiche que s'il y a des comptes jamais venus). Vérifié : 7/7 venus en démo → pas d'alerte (correct). | vérifié navigateur | non |
...
| **Fiche de suivi** (`/portefeuille/[id]/fiche`) vérifiée à l'écran : identité, investissement/valo, situation financière OHADA (vs N-1 et budget), navigation par période, export PDF. **Correctif** : le titre était figé « trimestrielle » alors que la période suit la cadence (mensuelle par défaut) → rendu dynamique via `periodKindLabel` (mensuelle / trimestrielle / annuelle). Vérifié « FICHE DE SUIVI MENSUELLE » sur juil. 2026. | vérifié navigateur | non |
