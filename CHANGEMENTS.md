...
| **Conversion dossier → participation : transfert de TOUT le dossier** (migration **0043**). S'ajoutent aux données déjà reprises (promoteur+éval, notes, KPIs, ESG, contacts, documents) : la **due diligence** (dd_items → société, affichée en lecture seule dans l'onglet *Origine / instruction*, groupée par domaine) et les **passages/décisions de comité** (committee_passages **déplacés** vers `company_id`, deal_id retiré → chronologie de l'onglet *Décisions*, la décision d'entrée en tête). Backfill des conversions démo existantes inclus. Vérifié à l'écran (BatiPro : 2 décisions reprises, 12 points de DD). | **fait** | **0043** |
