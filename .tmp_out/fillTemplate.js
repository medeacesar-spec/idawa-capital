...
function unzipAll(buf) {
    ...
    if (eocd < 0)
        throw new Error("Modèle illisible : archive .xlsx invalide.");
...
function parseRef(ref) {
    ...
    if (!m)
        throw new Error(`Référence de cellule invalide : ${ref}`);
    let index = 0;
    for (const ch of m[1])
 ...
 */
function applyToSheet(xml, values) {
    const byRow = new Map();
