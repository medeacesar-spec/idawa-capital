...
export async function getAuditForExport(): Promise<AuditEntry[]> {
  ...
  const rows: { id: string; table_name: string; row_id: string | null; action: string; label: string | null; actor_id: string | null; at: string | null }[] = [];
  const PAGE = 1000;
  for (let from = 0; ; from += PAGE) {
    const { data } = await supabase.from("audit_log").select("id, table_name, row_id, action, label, actor_id, at").order("at", { ascending: false }).range(from, from + PAGE - 1);
    const batch = data ?? [];
    rows.push(...batch);
  ...
  return rows.map((r) => ({ id: r.id, table: r.table_name, action: r.action, label: r.label, actor: r.actor_id ? pMap.get(r.actor_id) ?? "—" : "Système", at: r.at }));
}
