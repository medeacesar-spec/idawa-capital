...
export type CommitteePassage = { id: string; committeeType: string; sessionDate: string | null; decision: string | null; conditions: string | null; participants: string | null; status: string; validatedBy: string | null; validatedAt: string | null; docs: CommitteeDoc[] };
...
export async function getDealDetail(id: string): Promise<DealDetail | null> {
  ...
  const { data: d } = await supabase
    .from("deals")
    .select("id, company_name, stage, status, amount, probability, valuation_pre, ownership_target, thesis, program_id, primary_sub_sector_id, investment_officer_id, analyst_id, expected_close")
    .eq("id", id).single();
  if (!d) return null;
  ...
  const [progRes, subRes, offRes, anaRes, comRes, contactRes, docRes, convRes] = await Promise.all([
    ...
    supabase.from("committee_passages").select("id, committee_type, session_date, decision, conditions, participants, status, validated_by, validated_at").eq("deal_id", id).order("session_date"),
  ...
  return {
    id: d.id, companyName: d.company_name, stage: d.stage, status: d.status ?? null, amount: Number(d.amount ?? 0),
    ...
    committees: (comRes.data ?? []).map((c) => ({ id: c.id, committeeType: c.committee_type, sessionDate: c.session_date, decision: c.decision, conditions: c.conditions, participants: c.participants, status: c.status ?? "Proposée", validatedBy: c.validated_by ? (users.find((u) => u.id === c.validated_by)?.name ?? "—") : null, validatedAt: c.validated_at ?? null, docs: (comDocs ?? []).filter((doc) => doc.committee_id === c.id).map((doc) => ({ id: doc.id, title: doc.title, storagePath: doc.storage_path })) })),
