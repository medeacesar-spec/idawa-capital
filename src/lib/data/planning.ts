...
export type DdItem = { id: string; domain: string; item: string; status: string; note: string | null; assigneeId: string | null; dueDate: string | null; createdAt: string | null };
...
export async function getDdItems(entityType: "deal" | "company", entityId: string): Promise<DdItem[]> {
  ...
  const { data } = await supabase.from("dd_items").select("id, domain, item, status, note, assignee_id, due_date, created_at").eq("entity_type", entityType).eq("entity_id", entityId).order("created_at");
  return (data ?? []).map((x) => ({ id: x.id, domain: x.domain, item: x.item, status: x.status, note: x.note, assigneeId: x.assignee_id, dueDate: x.due_date ?? null, createdAt: x.created_at ?? null }));
}
