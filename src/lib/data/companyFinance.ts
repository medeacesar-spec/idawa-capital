...
export type FlowRow = {
  id: string; flowDate: string | null; type: string | null; amount: number | null;
  note: string | null; method: string | null;
  /** Renseignés pour les valorisations seulement : cadence annuelle et validation. */
  fiscalYear: number | null; status: string | null; validatedAt: string | null;
};
...
export async function getCompanyFinance(companyId: string): Promise<CompanyFinance> {
  ...
  const [fRes, flRes, capRes] = await Promise.all([
    ...
    supabase.from("company_flows").select("id, flow_date, type, amount, note, method, fiscal_year, status, validated_at").eq("company_id", companyId).order("flow_date", { ascending: false }),
  ...
  return {
    ...
    flows: (flRes.data ?? []).map((x) => ({ id: x.id, flowDate: x.flow_date, type: x.type, amount: x.amount != null ? Number(x.amount) : null, note: x.note, method: x.method, fiscalYear: x.fiscal_year ?? null, status: x.status ?? null, validatedAt: x.validated_at ?? null })),
