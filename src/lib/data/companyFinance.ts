...
export type FlowRow = { id: string; flowDate: string | null; type: string | null; amount: number | null; note: string | null; method: string | null };
...
export async function getCompanyFinance(companyId: string): Promise<CompanyFinance> {
  ...
  const [fRes, flRes, capRes] = await Promise.all([
    ...
    supabase.from("company_flows").select("id, flow_date, type, amount, note, method").eq("company_id", companyId).order("flow_date", { ascending: false }),
  ...
  return {
    ...
    flows: (flRes.data ?? []).map((x) => ({ id: x.id, flowDate: x.flow_date, type: x.type, amount: x.amount != null ? Number(x.amount) : null, note: x.note, method: x.method })),
