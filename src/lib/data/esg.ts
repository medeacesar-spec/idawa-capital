...
export type EsgImpact = { id: string; dimension: string | null; score: number | null; maxScore: number | null; note: string | null };
...
export async function getEsg(entityType: "deal" | "company", entityId: string): Promise<EsgData> {
  ...
  const [aRes, actRes, impRes] = await Promise.all([
    ...
    supabase.from("esg_impact_ratings").select("id, dimension, score, max_score, note").eq("entity_type", entityType).eq("entity_id", entityId),
  ]);
  ...
  return {
    ...
    impacts: (impRes.data ?? []).map((x) => ({ id: x.id, dimension: x.dimension, score: x.score != null ? Number(x.score) : null, maxScore: x.max_score != null ? Number(x.max_score) : null, note: x.note ?? null })),
  };
}
