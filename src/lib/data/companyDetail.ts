...
export async function getCompanyDetail(id: string): Promise<CompanyDetail | null> {
  ...
  const [suivi, esg, finance, kpis, kpiLibrary, valueCreation, instruments, users] = await Promise.all([getSuivi("company", id), getEsg("company", id), getCompanyFinance(id), getKpis("company", id), getKpiLibraryForEntity("company", id), getValueCreation("company", id), getInstruments(id), getFundUsers()]);
