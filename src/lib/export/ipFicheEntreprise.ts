...
async function buildOneCompany(companyId: string, endYear: number, endQ: number, quarterCount = 12): Promise<{ sheets: Sheet[]; context: FicheContext }> {
  ...
  const figees: CellValue[][] = [
    ...
    ["Année création", null],
    ["Secteur", sector],
    ["Ville", (c.country as string) ?? null],
    ["Stade de développement", null],
  ...
  const todo: Sheet = {
    ...
    rows: [
      ...
      ["Année de création · Ville · Stade de développement", "Champs non présents sur la fiche société — à ajouter (migration)."],
