    ...
    <div>
      <div style={panel}>
        ...
        <Field label="Méthodes de valorisation à l'acquisition" hint="Plusieurs méthodes possibles">
          <Chips canEdit={canEdit} options={VALUATION_METHODS} selected={vEntry}
        ...
        <Field label="Méthodes de valorisation à date" hint="Plusieurs méthodes possibles">
          <Chips canEdit={canEdit} options={VALUATION_METHODS} selected={vCurrent}
      ...
      <div style={panel}>
        ...
        <Field label="Scénarios de sortie envisagés" hint="Plusieurs pistes possibles — cochez toutes celles étudiées">
          <Chips canEdit={canEdit} options={EXIT_SCENARIOS} selected={exits}
            onToggle={(next) => { setExits(next); save({ exit_scenarios: next.length ? next : null }); }} />
        </Field>
        <Field label="Année de sortie attendue">
          <Input readOnly={!canEdit} disabled={!canEdit} type="number" value={f.exitYear} onChange={(e) => set("exitYear", e.target.value)}
        ...
        <Field label="Modalités de sortie" hint="Put option ? Avec quelle formule de prix ? Cession à un tiers identifié ? Clauses du pacte…">
          <Textarea readOnly={!canEdit} disabled={!canEdit} rows={4} value={f.exitStrategy} onChange={(e) => set("exitStrategy", e.target.value)}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <Field label="Multiple de sortie attendu">
            <Input readOnly={!canEdit} disabled={!canEdit} type="number" value={f.exitMultipleTarget} onChange={(e) => set("exitMultipleTarget", e.target.value)}
              onBlur={() => save({ exit_multiple_target: num(f.exitMultipleTarget) })} placeholder="Ex : 2.5" />
          </Field>
          <Field label="TRI de sortie attendu (%)">
            <Input readOnly={!canEdit} disabled={!canEdit} type="number" value={f.exitIrrTarget} onChange={(e) => set("exitIrrTarget", e.target.value)}
...
function Chips({ options, selected, onToggle, canEdit = true }: { options: string[]; selected: string[]; onToggle: (next: string[]) => void; canEdit?: boolean }) {
  return (
