...
export default function TodoBanner({ data }: { data: TodoData }) {
  if (data.total === 0) {
    return (
      <div className="card" style={{ padding: "13px 18px", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span style={{ fontSize: 13, color: "var(--text-2)" }}>Rien en retard. Tout est à jour.</span>
      </div>
    );
  }
  return (
    <div className="card" style={{ padding: "16px 20px", marginBottom: 16, borderLeft: "4px solid var(--camel)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 12 }}>
        ...
        <span className="badge" style={{ background: "var(--rework-bg)", color: "var(--rework)" }}>{data.total}</span>
      </div>
